    ...
    for (const item of items) {
      ...
      for (const filaReal of curvaCompleta) {
        ...
        if (item.sizes.includes(tallaDeEstaFila)) {
          // 🎨 Diferenciador Visual: Lo pintaremos de un color Cyan claro para distinguirlo de las urgencias de stock (amarillo)
          row.eachCell((cell) => { cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF00FFFF' } }; });
        }
        currentRow++;
      }
    }
